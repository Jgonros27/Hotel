...
class ImagenCabanasDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($imagenCabana) {
                // Renderiza las acciones usando una vista
                return view('admin.imagenCabanas.actions', compact('imagenCabana'));
            })
            ->addColumn('tipo_cabana', function ($imagenCabana) {
                // Accede al nombre del tipo de cabaña a través de la relación
                return $imagenCabana->tipoCabana->nombre;
            })
            ->addColumn('imagen', function ($imagenCabana) {
                // Crea un botón para ver la imagen
                $nombreCabana = $imagenCabana->tipoCabana->nombre;
                $imageUrl = asset('images/imagenes_cabanas/' . $imagenCabana->url);
                return "<button class='btn btn-primary btn-sm' onclick=\"openModal('{$imageUrl}','{$nombreCabana}')\">Ver imagen</button>";
            })
            ->setRowId('id')
     ...
     */
    public function query(ImagenCabana $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
                    ->setTableId('imagencabanas-table') // Define el ID de la tabla
    ...
    {
        return 'ImagenCabanas_' . date('YmdHis'); // Define el nombre de archivo para exportar
    }
}
