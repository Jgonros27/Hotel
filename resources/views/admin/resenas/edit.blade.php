...
<form action="{{ route('resenas.update', $resena->id) }}" method="POST" class="w-50">
    @csrf
    @method('PATCH')
    <div class="form-group">
        ...
        <select class="form-control js-example-basic-single" name="usuario" id="usuario">
            ...
            @foreach ($usuarios as $usuario)
                <option value="{{ $usuario->id }}" {{ $resena->id_usuario == $usuario->id ? 'selected' : '' }}>
                    {{ $usuario->name }}</option>
            @endforeach
        </select>
        @error('usuario')
    ...
    <div class="form-group">
        ...
        <textarea name="comentario" class="form-control" id="comentario" rows="4">{{ $resena->comentario }}</textarea>
        @error('comentario')
            <div class="alert alert-danger d-flex align-items-center" role="alert">
                <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="danger:">
                    <use xlink:href="#exclamation-triangle-fill" />
