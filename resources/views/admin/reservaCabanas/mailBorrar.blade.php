...
<body>
    <div class="container">
        <h1>{{ __('mail.confirmacion2') }} {{ __('menu.hotel') }} {{ __('mail.borrar3') }}</h1>
        <div class="logo">
        ...
        @if (\Carbon\Carbon::now() <  \Carbon\Carbon::parse($data['fechaEntrada'])->subDays($data['diasCancelacion']))
            @if ($data['id_pago'] != "")
                <p><b>{{ __('mail.devuelto') }}</b></p>
            @else
                <p><b>{{ __('mail.devueltoContacta') }}</b></p>
            @endif
        @else
            <p><b>{{ __('mail.noDevuelto') }}</b></p>
        @endif
