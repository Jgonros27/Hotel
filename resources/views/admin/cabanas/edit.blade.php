...
<form action="{{route('cabanas.update', $cabana->id_tipo_cabana)}}" method="POST" class="w-50">
    @csrf
    @method('PATCH')
    <div class="form-group">
        ...
        <select class="form-control js-example-basic-single" name="tipoCabana" id="tipoCabana">
            ...
            @foreach($tipoCabanas as $tipoCabana)
                <option value="{{ $tipoCabana->id }}" {{ $cabana->id_tipo_cabana == $tipoCabana->id ? 'selected' : '' }}>{{ $tipoCabana->nombre }}</option>
            @endforeach
        </select>
        @error('tipoCabana')
...
<a href="{{route('cabanas.store')}}" class="mt-3 btn btn-outline-secondary">
