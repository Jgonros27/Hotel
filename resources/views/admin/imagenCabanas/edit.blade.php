...
<form action="{{ route('imagenCabanas.update', $imagenCabana->id) }}" method="POST" class="w-50" enctype="multipart/form-data">
    @csrf
    @method('POST')
    <div class="form-group">
        ...
        <select class="form-control js-example-basic-single" name="tipoCabana" id="tipoCabana">
            ...
            @foreach($tipoCabanas as $tipoCabana)
                <option value="{{ $tipoCabana->id }}" {{ $imagenCabana->id_tipo_cabana == $tipoCabana->id ? 'selected' : '' }}>{{ $tipoCabana->nombre }}</option>
            @endforeach
        </select>
        @error('tipoCabana')
    ...
    <div class="form-group">
        ...
        <input type="hidden" name="imagenActual" value="{{ asset('images/imagenes_cabanas/' . $imagenCabana->url) }}">
        @error('imagenCabana')
            <div class="alert alert-danger d-flex align-items-center" role="alert">
                <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="danger:"><use xlink:href="#exclamation-triangle-fill"/></svg>
                <div>
    ...
    <div class="form-group">
        ...
        <input type="text" name="nombreImagen" class="form-control" value="{{ $imagenCabana->nombre_imagen }}" id="nombreImagen" placeholder="Introduce el nombre">
        @error('nombreImagen')
            <div class="alert alert-danger d-flex align-items-center" role="alert">
                <svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="danger:"><use xlink:href="#exclamation-triangle-fill"/></svg>
                <div>{{ $message }}</div>
